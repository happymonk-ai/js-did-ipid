Object.defineProperty(exports, '__esModule', { value: true });
exports.mockPem = exports.createMockIpfs = exports.mockDocument = exports.mockDid = exports.mockIpnsHash = exports.mockPath = exports.mockHash = void 0;
exports.mockHash = 'zdpuApA2CCoPHQEoP4nResbK2dq2zawFX3verNkMFmNbpDnXZ';
exports.mockPath = `/ipfs/${exports.mockHash}`;
exports.mockIpnsHash = 'QmUTE4cxTxihntPEFqTprgbqyyS9YRaRcC8FXp6PACEjFG';
exports.mockDid = `did:ipid:${exports.mockIpnsHash}`;
exports.mockDocument = {
    '@context': 'https://w3id.org/did/v1',
    id: exports.mockDid,
    created: '2019-03-19T16:52:44.948Z',
    updated: '2019-03-19T16:53:56.463Z',
    publicKey: [
        {
            id: `${exports.mockDid}#bqvnazrkarh`,
            type: 'myType',
            controller: 'myController',
            publicKeyHex: '1A2B3C',
        },
    ],
    authentication: [
        `${exports.mockDid}#bqvnazrkarh`,
    ],
    service: [
        {
            id: `${exports.mockDid};myServiceId`,
            type: 'myServiceType',
            serviceEndpoint: 'http://myserviceendpoint.com',
        },
    ],
};
const createMockIpfs = () => {
    let keychainKeys = [{ name: 'key1' }, { name: 'key2' }];
    const rmKey = (keyName) => {
        keychainKeys = keychainKeys.filter(({ name }) => name === keyName);
    };
    const addKey = (keyName) => {
        rmKey(keyName);
        keychainKeys.push({ name: keyName });
    };

    return {
        isOnline: jest.fn(() => true),
        name: {
            resolve: jest.fn(async () => ({ path: exports.mockPath })),
            publish: jest.fn(async () => { }),
        },
        dag: {
            put: jest.fn(async () => ({
                toBaseEncodedString: () => exports.mockHash,
            })),
            get: jest.fn(async () => ({
                value: exports.mockDocument,
            })),
        },
        key: {
            list: jest.fn(async () => keychainKeys),
            rm: jest.fn(async (keyName) => rmKey(keyName)),
            import: jest.fn(async (keyName) => addKey(keyName)),
        },
    };
};

exports.createMockIpfs = createMockIpfs;
exports.mockPem = `
-----BEGIN RSA PRIVATE KEY-----

ttrI1rZSmCBbKkicdvBsJo2c916giPwGpcGIzlrt83sW
-----END RSA PRIVATE KEY-----`;
